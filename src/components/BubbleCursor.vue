<template>
  <div class="bubble-wrapper" ref="wrapperRef" @mousemove="onMouseMove" @mouseenter="onMouseEnter" @mouseleave="onMouseLeave">
    <!-- Tutto il contenuto della sezione (Testi, Video di sfondo) finirà qui dentro -->
    <slot />
    
    <!-- Livello dedicato solo alle bolle, invisibile ai click -->
...
onMounted(() => {
  // Setup movimenti iper-fluidi per la bolla principale usando quickTo
...
const onMouseMove = (e) => {
  ...
  // Muovi la bolla principale
  xTo(x);
  yTo(y);

  // Frequenza di spawn delle bollicine (una ogni 40ms)
  const now = Date.now();
  if (now - lastSpawn > 40) {
    spawnTrail(x, y);
    lastSpawn = now;
...
const spawnTrail = (x, y) => {
  ...
  // Dimensione casuale organica
  const size = Math.random() * 12 + 6;
  bubble.style.width = `${size}px`;
  bubble.style.height = `${size}px`;
  
  // Centra la bolla sulle coordinate
  bubble.style.left = `${x - size/2}px`;
  bubble.style.top = `${y - size/2}px`;
  
  cursorLayerRef.value.appendChild(bubble);

  // Animazione GSAP: fluttua verso l'alto e rimpicciolisce scomparendo
  gsap.to(bubble, {
    y: `-=${Math.random() * 50 + 20}`,
    x: `+=${Math.random() * 30 - 15}`,
    scale: 0.1,
    ...
    ease: "power1.out",
    onComplete: () => bubble.remove() // Elimina l'elemento per non appesantire il DOM
  });
};
...
.bubble-wrapper {
  ...
  width: 100%;
  height: 100%;
  cursor: none; /* Nasconde il cursore di default del sistema operativo */
}
...
.main-bubble {
  ...
  height: 40px;
  border-radius: 50%;
  /* Stile Vetro Organico Lilla/Magenta */
...
/* Stili globali per le bollicine generate in JS */
:global(.trail-bubble) {
  position: absolute;
  border-radius: 50%;
